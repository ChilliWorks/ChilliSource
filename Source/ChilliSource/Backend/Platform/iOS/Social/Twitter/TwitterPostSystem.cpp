    ...
    namespace iOS
    {
		...
		bool TwitterPostSystem::Authenticate()
		{
			...
			if(mstrSavedOAuthTokenKey.empty() || mstrSavedOAuthTokenSecret.empty())
			{
				...
				if(RequestOAuthToken(strAuthoiseURL))
				{
					...
					if(Core::Application::Get()->GetPlatformSystemPtr()->CanCreateActivityWithInterface<ChilliSource::Social::TwitterAuthenticationActivity>())
					{
						mpAuthenticationView = Core::Application::Get()->GetPlatformSystemPtr()->CreateActivityWithInterface<ChilliSource::Social::TwitterAuthenticationActivity>();
		...
		bool TwitterPostSystem::TryPostUsingiOS(const Social::TwitterPostDesc & insDesc, const Social::TwitterPostSystem::PostResultDelegate & inResultCallback)
		{
			if([TWTweetComposeViewController canSendTweet])
            {
                ...
                {
                    std::string strPath = Core::Application::Get()->GetFileSystemPtr()->GetStorageLocationDirectory(insDesc.eLocalImageStorageLocation) + insDesc.strLocalImagePath;
