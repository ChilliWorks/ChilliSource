...
namespace ChilliSource
{
    namespace Core
    {
        ...
        //----------------------------------------------------------------------
        void ResourceManagerDispenser::SetResourceProviderOlds(const std::vector<ResourceProviderOld*> inProviders)
        {
			...
			for (std::vector<ResourceManager*>::iterator pMgr = mResourceManagers.begin(); pMgr != mResourceManagers.end(); ++pMgr) 
			{
				...
				std::vector<ResourceProviderOld*> UsefulResourceProviderOlds;
				
				for (std::vector<ResourceProviderOld*>::const_iterator pProv = inProviders.begin(); pProv != inProviders.end(); ++pProv) 
				{
					if ((*pProv)->CanCreateResourceOfKind(ResourceType)) 
					{
						UsefulResourceProviderOlds.push_back((*pProv));
					}
				}
				(*pMgr)->SetResourceProviderOlds(UsefulResourceProviderOlds);
			}
        }
        //----------------------------------------------------------------------
        //----------------------------------------------------------------------
