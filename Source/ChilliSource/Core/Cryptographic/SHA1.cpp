	...
	namespace Core
	{
		...
		bool CSHA1::HashFile(char *szFileName)
		{
			...
			for(i = 0; i < ulBlocks; i++)
			{
				...
				Update((u8 *)uData, SHA1_MAX_FILE_BUFFER);
			}
		...
		void CSHA1::Final()
		{
			...
			for(i = 0; i < 8; i++)
				finalcount[i] = (u8)((m_count[((i >= 4) ? 0 : 1)]
			...
			for(i = 0; i < 20; i++)
			{
				m_digest[i] = (u8)((m_state[i >> 2] >> ((3 - (i & 3)) * 8) ) & 255);
			}
		...
		void CSHA1::ReportHash(char *szReport, ReportType eReportType)
		{
			...
			else if(REPORT_DIGIT == eReportType)
			{
