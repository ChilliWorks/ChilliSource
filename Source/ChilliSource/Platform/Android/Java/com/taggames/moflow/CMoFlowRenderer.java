...
public class CMoFlowRenderer implements GLSurfaceView.Renderer 
{
	//------------------------------------------------------------------
	/// Renderer State
	///
	...
	private CMoFlowLoadingView mLoadingView = null;
	private boolean mbNewLaunchIntent = false;
	private boolean mbResetTime = false;
	//------------------------------------------------------------------
	/// Constructor
	//------------------------------------------------------------------
	public CMoFlowRenderer(CMoFlowLoadingView inLoadingView)
	{
    ...
    {	
    	if (CMoFlowActivity.GetActivity().IsActive() == true)
    	{
    ...
    private void Render(GL10 inGlContext)
    {
    	...
    	if (mbNewLaunchIntent == true)
    	{
    		...
    		Intent newIntent = CMoFlowActivity.GetActivity().getIntent();
    		if (newIntent != null)
    		{
    			CNativeInterfaceManager.GetSingleton().OnLaunchIntentReceived(newIntent);
    		}
    	}
    	
    	//start the frame.
    	((CCoreNativeInterface)CNativeInterfaceManager.GetSingleton().GetNativeInterface(CCoreNativeInterface.InterfaceID)).FrameBegin(fDeltaTime, mddwTotalRunningTime);
