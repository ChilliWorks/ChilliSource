//  
//  CAmazonIAPNativeInterface.java
//  moFlow
//  
...
import com.taggames.moflow.core.CHashCRC32;
...
//===========================================================
public class CAmazonIAPNativeInterface extends INativeInterface implements IResumeable
{
	//-----------------------------------------------------
	/// State
	///
	...
	/// Constants
	//-----------------------------------------------------
	public static CInterfaceIDType InterfaceID = new CInterfaceIDType("CAmazonIAPNativeInterface");
	...
	private volatile CDataStore mDataStore = null;
	...
	/// Constructor
	//-----------------------------------------------------
	public CAmazonIAPNativeInterface()
	{
	}
	//-----------------------------------------------------
	/// Is A
	...
	/// interface
	//-----------------------------------------------------
	@Override synchronized public boolean IsA(CInterfaceIDType inInterfaceType) 
	{
	...
	public void Init(final String instrPrivateKey, final String instrUDID)
	{
		...
		final CAmazonIAPNativeInterface thisClass = this;
		...
		Runnable task = new Runnable()
		{
			@Override public void run()
			{
				if (meState == State.UNINITIALISED)
				{
					...
					PurchasingManager.registerObserver(new CPurchasingObserver(mActivity, thisClass));
				}
			}
		};
	...
	public synchronized void RequestProductPurchase(final String instrProductID)
	{
		...
		else
		{
			...
			ProcessPurchase(CPurchaseTransaction.kdwPurchaseFailed, instrProductID, ItemType.CONSUMABLE, "", "", false);
		}
	}
	//-----------------------------------------------------
	/// Close Transaction
	...
	public synchronized void OnPurchaseResponse(final PurchaseResponse inResponse)
	{
		...
		switch (eStatus)
        {
	        case SUCCESSFUL:
	        {
	            ...
	            {
	            	ProcessPurchase(CPurchaseTransaction.kdwPurchaseSuccessful, receipt.getSku(), receipt.getItemType(), inResponse.getRequestId(), receipt.getPurchaseToken(), true);
	            }
	            else
	            {
	            	ProcessPurchaseForUser(inResponse.getUserId(), CPurchaseTransaction.kdwPurchaseSuccessful, receipt.getSku(), receipt.getItemType(), inResponse.getRequestId(), receipt.getPurchaseToken());
	            }
	            break;
	        }
	        default:
	        {
	        	ProcessPurchase(CPurchaseTransaction.kdwPurchaseFailed, mstrCurrentSKU, ItemType.CONSUMABLE, "", "", false);
	        	break;
	        }
        }
				...
				for (final Receipt receipt : inResponse.getReceipts()) 
				{
                    switch (receipt.getItemType()) 
                    {
                        case ENTITLED:
                        {
        		            if (inResponse.getUserId().equals(mstrUserID) == true)
        		            {
        		            	ProcessPurchase(CPurchaseTransaction.kdwPurchaseRestored, receipt.getSku(), receipt.getItemType(), inResponse.getRequestId(), receipt.getPurchaseToken(), false);
        		            }
        		            else
        		            {
        		            	ProcessPurchaseForUser(inResponse.getUserId(), CPurchaseTransaction.kdwPurchaseRestored, receipt.getSku(), receipt.getItemType(), inResponse.getRequestId(), receipt.getPurchaseToken());
        		            }
                        	break;
                        }
                        default:
	...
	{
		String strUserIDHash = Integer.toString(Math.abs(CHashCRC32.Generate(CStringUtils.StringToUTF8ByteArray(mstrUserID))));
		mDataStore = new CDataStore(kstrCacheNamePrefix + strUserIDHash + kstrCacheExtension, mstrPrivateKey, mstrUDID, mstrUserID);
	}
	//-----------------------------------------------------
	...
	private String BuildPurchaseReceipt(final String instrUserID, final String instrToken)
	{
		...
		{
			android.util.Log.e("MoFlow", CExceptionUtils.ConvertToString(e));
		}
		return strReceipt;
	}
	//-----------------------------------------------------
	...
	private void ProcessPendingTransactions()
	{
		...
		{
			final LinkedList<CPurchaseTransaction> astrTransactions = mDataStore.GetPendingPurchaseTransactions();
			if (astrTransactions.size() > 0)
			{
				for (final CPurchaseTransaction transaction : astrTransactions)
				{
					...
					if (dwResult == CPurchaseTransaction.kdwPurchaseSuccessful)
					{
						dwResult = CPurchaseTransaction.kdwPurchaseResumed;
					}
	...
	{
		if (indwResult == CPurchaseTransaction.kdwPurchaseSuccessful || indwResult == CPurchaseTransaction.kdwPurchaseRestored)
		{
			...
			CPurchaseTransaction transaction = new CPurchaseTransaction(indwResult, instrSKU, eProductType, instrRequestId, strReceipt);
		...
		else if (indwResult == CPurchaseTransaction.kdwPurchaseFailed && mbListeningForPurchaseTransactions == true)
		{
	...
	private void ProcessPurchaseForUser(final String instrUserID, int indwResult, final String instrSKU, final ItemType ineItemType, final String instrRequestId, final String instrPurchaseToken)
	{
		if (indwResult == CPurchaseTransaction.kdwPurchaseSuccessful || indwResult == CPurchaseTransaction.kdwPurchaseRestored)
		{
			...
			String strUserIDHash = "" + Math.abs(CHashCRC32.Generate(CStringUtils.StringToUTF8ByteArray(instrUserID)));
			CDataStore dataStore = new CDataStore(kstrCacheNamePrefix + strUserIDHash + kstrCacheExtension, mstrPrivateKey, mstrUDID, instrUserID);
