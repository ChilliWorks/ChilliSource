...
public class CFileUtils
{
	...
	public static final String GetExternalStorageDirectory()
	{
		...
		if (Environment.MEDIA_MOUNTED.equals(state))
			return CStringUtils.StandardisePath(Environment.getExternalStorageDirectory().toString());
	...
	public static boolean DoesFileExist(StorageLocation ineStorageLocation, final String instrFilepath)
	{
		...
		return DoesFileExist(CStringUtils.StandardisePath(strStorageLocation) + instrFilepath);
	}
	//-----------------------------------------------
	/// Read File
	///
	...
	public static byte[] ReadFile(StorageLocation ineStorageLocation, final String instrFilename)
	{
		...
		return ReadFile(CStringUtils.StandardisePath(strStorageLocation) + instrFilename);
	}
	//-----------------------------------------------
	/// Write File
	///
	...
	public static boolean WriteFile(StorageLocation ineStorageLocation, final String instrFilename, final byte[] instrFileContents)
	{
		...
		return WriteFile(CStringUtils.StandardisePath(strStorageLocation) + instrFilename, instrFileContents);
	}
	//-----------------------------------------------
	/// Remove File
	///
	...
	public static boolean RemoveFile(StorageLocation ineStorageLocation, final String instrFilename)
	{
		...
		return RemoveFile(CStringUtils.StandardisePath(strStorageLocation) + instrFilename);
	}
	//-----------------------------------------------
	/// Does File Exist
	///
	...
	{
		File file = CMoFlowActivity.GetActivity().getBaseContext().getFileStreamPath(instrFilepath);
	    if (file.exists() == true && file.isFile() == true)
	    {
	    	return true;
	    }
	...
	private static byte[] ReadFileInternal(final String instrFilename)
	{
		...
		try
		{
			if (DoesFileExistInternal(instrFilename) == true)
			{
				...
				try
				{
					File file = CMoFlowActivity.GetActivity().getBaseContext().getFileStreamPath(instrFilename);
					abyOutput = new byte[(int)file.length()];
					stream = new BufferedInputStream(CMoFlowActivity.GetActivity().openFileInput(instrFilename));
					int dwTotalRead = 0;
					while(dwTotalRead < abyOutput.length)
					{
						int dwRemaining = abyOutput.length - dwTotalRead;
	...
	private static boolean WriteFileInternal(final String instrFilename, final byte[] instrFileContents)
	{
		...
		try
		{
			...
			try
			{
				stream = new BufferedOutputStream(CMoFlowActivity.GetActivity().openFileOutput(instrFilename, CMoFlowActivity.MODE_PRIVATE));
				stream.write(instrFileContents);
				bSuccess = true;
			}
	        catch (Exception e)
	...
	private static boolean RemoveFileInternal(final String instrFilename)
	{
		if (DoesFileExistInternal(instrFilename) == true)
		{
			return CMoFlowActivity.GetActivity().deleteFile(instrFilename);
		}
